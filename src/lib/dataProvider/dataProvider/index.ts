...
import { CrudOperators, CrudFilters, CrudSorting, DataProvider, HttpError, CrudFilter } from '@refinedev/core'
import { set, setWith } from 'lodash'
import HACK_TYPE from '../hackType'
...
const handleError = (error: object | Error) => {
  const customError: HttpError = {
    ...error,
    message: Array.isArray(error) ? error.map((p) => p.message).join(', ') : JSON.stringify(error),
    statusCode: 200,
  }
...
const dataProvider = (client: NhostClient, options?: NHostDataProviderOptions): Required<DataProvider> => {
  ...
  return {
    getOne: async ({ resource, id, meta }) => {
      ...
      const { query, variables } = gql.query({
        operation: HACK_TYPE[operation] ?? operation,
        variables: {
          id: {
            value: id,
        ...
        fields: meta?.fields ?? meta?.getOne?.fields,
      })

      const { data, error } = await client.graphql.request(query, variables)
    ...
    getMany: async ({ resource, ids, meta }) => {
      ...
      const { query, variables } = gql.query({
        operation: HACK_TYPE[operation] ?? operation,
        fields: meta?.fields ?? meta?.getMany?.fields,
    ...
    getList: async ({ resource, sorters, filters, pagination, meta }) => {
      ...
      const { query, variables } = gql.query([
        {
          operation: HACK_TYPE[operation] || operation,
          fields: meta?.fields ?? meta?.getList?.fields,
          variables: {
            ...(mode === 'server'
              ? {
                  limit,
        ...
        {
          operation: HACK_TYPE[aggregateOperation] || aggregateOperation,
    ...
    create: async ({ resource, variables, meta }) => {
      ...
      const { query, variables: gqlVariables } = gql.mutation({
        operation: HACK_TYPE[insertOperation] || insertOperation,
        variables: {
          object: {
            type: insertType,
            value: variables,
      ...
      const { data, error } = await client.graphql.request(query, gqlVariables)
    ...
    update: async ({ resource, id, variables, meta }) => {
      ...
      const { query, variables: gqlVariables } = gql.mutation({
        operation: HACK_TYPE[updateOperation] || updateOperation,
        variables: {
          pk_columns: {
            type: pkColumnsType,
            value: {
    ...
    deleteOne: async ({ resource, id, meta }) => {
      ...
      const { query, variables } = gql.mutation({
        operation: HACK_TYPE[deleteOperation] || deleteOperation,
        variables: {
          id: {
            value: id,
            type: getIdType(resource),
